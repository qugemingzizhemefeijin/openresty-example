...
function KCmd(range,w) {
	this.init(range,w);
}
_extend(KCmd, {
	init : function(range,w) {
	...
	insertimage : function(url, title, width, height, border, align) {
		...
		var html = (self.kind.options.imageCenter?'<p align="center">':'');
		...
		html += '/>'+(self.kind.options.imageCenter?'</p>':'');
...
function _cmd(mixed,w) {
	if (mixed.nodeName) {
		var doc = _getDoc(mixed);
		mixed = _range(doc).selectNodeContents(doc.body).collapse(false);
	}
	return new KCmd(mixed,w);
}
K.cmd = _cmd;
...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	var arr = [
		...
		'<link type="text/css" rel="stylesheet" href="' + themesPath +'bodycss.css?v='+(typeof JSKinds == "undefined" ? '':JSKinds)+'" />'
	];
	if (!_isArray(cssPath)) {
		cssPath = [cssPath];
	}
...
_extend(KEdit, KWidget, {
	init : function(options) {
		...
		self.imageCenter = _undef(options.imageCenter, false);
		self.imageText = _undef(options.imageText, false);
...
_plugin('core', function(K) {
	...
	self.afterCreate(function() {
		...
		function movePastedData() {
			...
			if (self.pasteType === 2) {
				//alert(html);
				if (/schemas-microsoft-com|worddocument|mso-\w+|Mso\w+/i.test(html)) {
					html = _clearMsWord(html, self.filterMode ? self.htmlTags : K.options.htmlTags);
				} else {
					html = html.replace(/font-family:([^;]+);/ig,'')
					.replace(/font-size:([^;]+);/ig,'')
					.replace(/style="([\s]*)"/ig,'')
					.replace(/([ ]*)lang="EN-US"/ig,'')
					.replace(/<span[^>]*>(&nbsp;)*<\/span>/ig,'')
					.replace(/<b><\/b>/ig,'');
			...
			html = html.replace(/<p><span><\/span><\/p>/g,'<br/>');
			html = html.replace(/<p><b><span><\/span><\/b><\/p>/g,'<br/>');
			html = html.replace(/<p><span><b><\/b><\/span><\/p>/g,'<br/>');
