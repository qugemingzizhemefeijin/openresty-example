...
var App = function () {
	...
	var modalDefHtml = {title : '提示',show:null,showTitle : true,showCloseButton : true,hide : null , width:-1};
	var modalDefLoad = {title : '提示',show:null,showTitle : true,showCloseButton : true,hide : null , width:-1};
			...
			if(r){
				...
				if(x=='0'){
					...
					App.Modal.alert(d.length==0?'您当前的登录信息已过期，请重新登录！':d,{confirm : function(){
						top.location.href=_contextPath + '/login';
					}});
				}else if(x=='1'){
					closeModel();
					App.Modal.alert(d);
